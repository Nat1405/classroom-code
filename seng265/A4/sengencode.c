...
int run_length_decode(int *code, int size, int *msg)
{
    ...
    /* Whenever we see a zero, place count 1's into the output array */
    int j = 0; /* Index into msg */
    int i = 0; /* Index into code */
    int flag = 0;
    for (i = 0; i<size; i++){
      /* If we just expanded a zero, skip the next element */
      if (flag){
        flag = 0;
        continue;
      }
      ...
      if (code[i] == 0){
        /* Copy code[i+1] 1's into msg */
        int counter = code[i+1];
        flag = 1;
        int k = 0;
        for (k=0; k<counter; k++){
          msg[j] = 1;
...
int run_length_encode(int *msg, int size, int *code)
{
    ...
    for (i = 0; i<size; i++){
      ...
      /* Copy is triggered by non-1 */
      else{
        /* Four states when we hit a non-1 value. count=0, count = 1, count = 2, or count > 2.*/
        if (count == 0){
          /* Just copy the single value over */
          code[j] = msg[i];
          j++;
        }
        else if (count == 1){
          /* Copy a single one then the value over*/
          code[j] = 1;
          j++;
          code[j] = msg[i];
          j++;
          ...
          /* Count was greater than two; do real run length encoding*/
          code[j] = 0;
          j++;
          code[j] = count;
          j++;
    ...
    /* Handle case where 1's go to end of array */
    if (count == 1){
      code[j] = 1;
      j++;
    }
...
int check_run_length(int *msg, int size, int *code)
{
    ...
    /* Let's assume that rle messages are never less than
    half the size of mtf encoded messages. */
    int *msg2 = (int *) malloc(sizeof(int)*size*2);
    if (!msg2){
      abort();
    }
